...
public class Mailer {
  ...
  public Mailer(Environment env) {
    if (instance == null) {
      ...
      try {
        if (env.getProperty("mail.smtp.logoLocation") != null
            && !env.getProperty("mail.smtp.logoLocation").isBlank())
          emailProps.setProperty(
              ...
              "data:image/png;base64,"
                  + Base64.getEncoder()
                      .encodeToString(
                          Mailer.class
                              .getResourceAsStream(env.getProperty("mail.smtp.logoLocation"))
                              .readAllBytes()));
      } catch (IOException e) {
        e.printStackTrace();
      }
  ...
  private String fillTemplate(String templateName, Properties props) throws IOException {
    String message =
        new String(Mailer.class.getResourceAsStream("/email/" + templateName).readAllBytes());
    ...
    for (String variable : matches) {
      if (props.getProperty(variable) != null) {
        ...
        message =
            message.replaceAll("\\{\\{" + variable + "\\}\\}", emailProps.getProperty(variable));
      } else {
  ...
  public void verifyEmail(User user) {
    ...
    props.setProperty(
        "link",
        instance.emailProps.getProperty("urlPath")
            + "verify-email?jwt="
            + Utils.createJWT(user, 1000L * 60 * 60 * 24));
    instance.sendMail(
        user.getEmail(),
        "Verify Email | " + instance.emailProps.getProperty("appName"),
        "verify-email.html",
        props);
  }
  ...
  public void resetPassword(User user) {
    ...
    instance.sendMail(
        user.getEmail(),
        "Reset Password | " + instance.emailProps.getProperty("appName"),
        "reset-password.html",
        props);
  }
  ...
  public void unlockAccount(User user) {
    ...
    instance.sendMail(
        user.getEmail(),
        "Locked Account | " + instance.emailProps.getProperty("appName"),
        "unlock-account.html",
        props);
  }
  ...
  public void sendContact(Contact contact) {
    ...
    instance.sendMail(
        instance.emailProps.getProperty("supportEmail"),
        "Contact Request | " + instance.emailProps.getProperty("appName"),
        "contact-admin.html",
        props);
  }
}
