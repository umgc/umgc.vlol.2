...
public class UserAllergyController {
    ...
    public ModelAndView viewAllergyList(@PathVariable(name = "id", required=false) Long id, Model model, Principal principal) {
    ...
    @RequestMapping(value = {"/user/save-allergy", "/user/save-allergy/{id}"}, method = RequestMethod.POST)
    public String saveAllergy(@PathVariable(name = "id", required=false) Long id, @Valid UserAllergy allergy, BindingResult bindingResult, Model model) {
        User user = Utils.getIfAuthorizedForUser(userService, id, true);
        if(user == null)
    ...
    public ModelAndView viewAddAllergyPage(@PathVariable(name = "id", required=false) Long id, Model model) {
    ...
    public ModelAndView viewEditAllergyPage(@PathVariable(name = "id", required=false) Long id, @PathVariable(name = "allergyId") Long allergyId, Model model) {
    ...
    @RequestMapping(value = {"/user/delete-allergy/{allergyId}", "/user/delete-allergy/{id}/{allergyId}"})
    public String deleteAllergy(@PathVariable(name = "id", required=false) Long id, @PathVariable(name = "allergyId") Long allergyId) {
        // Check if this user can edit the requested user
        User user = Utils.getIfAuthorizedForUser(userService, id, true);
        if(user == null)
            return "redirect:/login";
        ...
        if(id==null)
            ...
            return "redirect:/user/allergies/"+id;
    }
}
