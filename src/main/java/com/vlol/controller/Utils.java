...
public class Utils {
  ...
  public static User getIfAuthorizedForUser(
      UserService userService, Long userId, Boolean editable) {
    Authentication auth = SecurityContextHolder.getContext().getAuthentication();
    if (auth.getPrincipal() != "anonymousUser") {
  ...
  public static String createJWT(User user, Long expiry) {
    JWTCreator.Builder jwtBuilder = JWT.create().withClaim("userId", user.getUserId());
    if (expiry != null)
  ...
  public static Boolean verifyJWT(User user, String jwt) {
    JWTVerifier verifier = JWT.require(algorithm).withClaim("userId", user.getUserId()).build();
    try {
      verifier.verify(jwt);
      return true;
